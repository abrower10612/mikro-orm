...
export const create = async (req: Request, res: Response) => {
  ...
  const missingInputs = [];
  !title && missingInputs.push('title');
  !description && missingInputs.push('description');
  if (missingInputs.length) {
    return res.json({ message: `Missing inputs: ${missingInputs}` });
  }
...
export const updateOne = async (req: Request, res: Response) => {
  ...
  if (!title && !description)
    return res.json({
      message: 'Please change title or description to update',
    });
  try {
    const orm = await MikroORM.init(mikroConfig);
    const post = await orm.em.findOne(Post, { id });
